...
# Run the asadmin delete-jvm-options command. 
#
...
OUTPUT=$(asadmin --user="${PAYARA_ADMIN_USER}" --passwordfile="$PASSWORD_FILE" delete-jvm-options $JVM_OPTIONS)
STATUS=$?
rm "$PASSWORD_FILE"
if [ "$STATUS" -ne 0 ]
  then
...
# show output
# should end with: Command delete-jvm-options executed successfully
#
# Note that if the exact same options do not exist, 
# the command will still succeed with an message like: "No jvm-options were deleted (perhaps they never existed in the configuration)"
echo "$OUTPUT"