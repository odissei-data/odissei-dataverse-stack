...
if [ ! -d "$EXPORTER_DIR_DST" ]; then
  exit_with_error_message "No $EXPORTER_DIR_DST directory; exiting!"
fi
...
if [ ! -f "$NEW_JAR_FILE" ]; then
  exit_with_error_message "No $NEW_JAR_FILE file; exiting!"
fi
    ...
    while true; do
        ...
        if [ "$VERSION_STATUS_CODE" -eq 200 ]; then
            ...
            if [ "$retries" -ge "$max_retries" ]; then
                ...
                exit_with_error_message "Dataverse API did not become responsive in time; exiting!"
            fi
...
echo "Upgrade complete! Please check the dataverse logs to monitor the reExportAll process. Note that it may take a while for all exports to be updated."
# Get back to where we were
cd "$CURRENT_DIR" || exit 1
exit 0
